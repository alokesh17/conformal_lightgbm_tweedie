...
ssdglm.autograd_modified <- function(y = NULL,
                            x = NULL,
                            z = NULL,
                            beta.init = NULL,
                            gamma.init = NULL,
  ...
  A.gamma.mat = ztz + (reg.factor + 1) * diag(q) #reg.factor=0
  ...
  for(i in 1:niter){
    ...
    beta.draw =  beta + tau.beta * A.beta %*% nabla.beta/2 + chol(tau.beta * A.beta) %*% rnorm(p) #tau.beta=.1
    xb.draw = as.vector(x %*% beta.draw) #beta.draw=
    ...
    t1 = (1 - alpha.beta) * 1/sqrt(nu0) * tmp^(1/nu0) ##nu0=1e-4
    t2 = alpha.beta * tmp
    zeta.pr = t2/(t1 + t2)
    ...
    dldphi = delphi(y = y, xi = xi, mu = exp(xb), phi = exp(zg)) ##need delphi
    nabla.gamma = - gamma/(sigma2.gamma * zeta.gamma) + crossprod(z, exp(zg) * dldphi)
    
    gamma.draw =  gamma + tau.gamma * A.gamma %*% nabla.gamma/2 + chol(tau.gamma * A.gamma) %*% rnorm(q) ##tau.gamma<-1e-1
    ...
    #############
    # update xi # power parameter
    #############
    xi.draw = tau.xi * rnorm(1) + xi ##tau.xi<-1e-1
      ...
      if(verbose){
        if(i>nburn){
          cat("Iteration::", i, "Acceptance:", accept.p,"\n",
              ...
              "Gamma::", round(apply(matrix(res_gamma[nburn:i,],ncol=1), 2, median), digits = (digits - 1)), ", tuning.gamma = ", round(tau.gamma, 2),"\n",##change
  ...
  # Inference #
  #############
  sample.win = seq((nburn + 1), niter, by = thin) ##thin<-1, digits=2
  ...
  gamma.est = cbind.data.frame(MAP = round(apply(matrix(res_gamma[sample.win],ncol=1), 2, function(s){ den = density(s); ind = which.max(den$y); den$x[ind]}), digits = digits),
                               median = round(apply(matrix(res_gamma[sample.win],ncol=1), 2, median), digits = digits),
                               mean = round(mean(res_gamma[sample.win]), digits = digits),
                               sd = round(sd(res_gamma[sample.win]), digits = digits),
